...
export default class MoviesUpdate extends Component {
    ...
    handleChangeInputName = async (e) => {
        const name = e.target.value;
        this.setState({name});
    };

    handleChangeInputRating = async (e) => {
        const rating = e.target.validity.valid ? e.target.value : this.state.rating;
        this.setState({rating});
    };

    handleChangeInputTime = async (e) => {
    ...
    render() {
        ...
        return (
            <Wrapper>
                ...
                <Label>Rating: </Label>
                <InputText type="number" step="0.1" lang="en-US" min="0" max="10" pattern="[0-9]+([,\.][0-9]+)?" value={rating} onChange={this.handleChangeInputRating} />
                <Label>Time: </Label>
